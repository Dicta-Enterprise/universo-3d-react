...
export default function OverlayLanding({ open, onClose, planeta, color = "#62d4ff" }) {
  ...
  const renderResumen = () => {
    ...
    const str = val.trim();
    const hasDangerous = /<(script|style|link|iframe|meta|head|html|body|form|input|button)[\s>]/i.test(str);
    const hasLayoutTags = /<\/?(section|article|header|footer|main)[\s>]/i.test(str);
  ...
  const beneficios = Array.isArray(planeta?.beneficios)
    ? planeta.beneficios
    : (planeta?.beneficios ? Object.values(planeta.beneficios) : []);

  const peligros = Array.isArray(planeta?.peligros)
    ? planeta.peligros
    : (planeta?.peligros ? Object.values(planeta.peligros) : []);

  return createPortal(
    <div className="overlay-backdrop" onClick={handleBackdropClick} style={{ "--accent": color }}>
      <div className="overlay-container" onClick={stop}>
        ...
        <div className="overlay-hero">
          ...
          <button className="overlay-close" onClick={onClose}>Cerrar ✖</button>
            ...
            <div className="overlay-grid">
              {beneficios.map((b, i) => (
                <div key={i} className="overlay-card">
                  {b.titulo && <h3 className="overlay-card-title">{b.titulo}</h3>}
                  {b.descripcion && <p className="overlay-card-text">{b.descripcion}</p>}
        ...
        {!!peligros.length && (
          <section className="overlay-section">
            ...
            <div className="overlay-grid">
              {peligros.map((p, i) => (
                <div key={i} className="overlay-card">
                  {p.nombre && <h3 className="overlay-card-title">{p.nombre}</h3>}
                  {p.descripcion && <p className="overlay-card-text">{p.descripcion}</p>}

                  {"nivelRiesgo" in p && <p className="overlay-card-text"><b>Nivel:</b> {p.nivelRiesgo}</p>}
                  {"temperatura" in p && <p className="overlay-card-text"><b>Temperatura:</b> {p.temperatura}</p>}
                  {"villano" in p && <p className="overlay-card-text"><b>Villano:</b> {p.villano}</p>}

                  <ExtraFields obj={p} exclude={["nombre", "descripcion", "nivelRiesgo", "temperatura", "villano", "cta"]} />

                  {p.cta && (
                    <div className="overlay-cta">
                      <button type="button" onClick={() => onClose?.()}>{p.cta}</button>
                    </div>
                  )}
                </div>
              ))}
